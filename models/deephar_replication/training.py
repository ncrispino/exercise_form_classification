...
import wandb
wandb.init(project='deephar_replication')
...
# for output in train_dataloader:
#     img = output['frame']
...
#     img = img.unsqueeze(1) # add T=1 channel
#     visibility, _, joints = pose_model(img) # B x 1 x N_J, _, B x pose_dim x N_J
#     joint_vis_pred = torch.concat([joints, visibility], dim=1)    
#     print(joint_vis_pred)
#     print(joint_vis_true)    
#     print(loss_fn(joint_vis_pred, joint_vis_true))
#     quit()
...
def joint_training(model, loss_fn, optimizer, num_epochs, train_loader):
    for epoch in range(1, num_epochs + 1):
        for output in train_loader:
            ...
            wandb.log({'images': [wandb.Image(im) for im in imgs]})
...
# Overfit one batch (starting with batch_size=8 as 20 is too much to handle for my CPU)
...
optimizer = optim.RMSprop(pose_model.parameters(), lr=wandb.config.lr)

wandb.watch(pose_model)
